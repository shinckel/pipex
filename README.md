...
| **`empty string`**| `./pipex grocery_list.txt " " "" sorted.txt` | When using empty strings as arguments, the perror will produce a `success` message. Therefore, I developed a flag to treat errors differently in those cases `void	msg_error(char *err, int empty)`. When empty is true, I will force an error description through `errno = 1;` |
[MultiPass](https://multipass.run/install) | `Ubuntu` | use ubuntu OS to test with Valgrind (so I can check if there are leaks or opened fds in my code). I chose to use Multipass for creating an Ubuntu VM in my MacOS `multipass start pipex` `multipass shell pipex` `multipass stop pipex` ![valgrind](https://github.com/shinckel/pipex/assets/115558344/cbdf4e14-512d-4c00-be02-ac17e2f01da7) |
