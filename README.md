...
| **`open()`** | `int open(const char *pathname, int flags)`, it returns a file descriptor | open and possibly create a file. You must add the file permissions! In my case I used `0644`(octal format) = owner has read and write permissions `4 + 2 = 6`, group has read-only permission `4`, others also have read-only permission `4` |
...
[MultiPass](https://multipass.run/install) | `Ubuntu` | use ubuntu OS to test with valgrind (so I can check if there are leaks or opened fds in my code). I chose to use Multipass for creating an Ubuntu VM in my MacOS ![valgrind](https://github.com/shinckel/pipex/assets/115558344/cbdf4e14-512d-4c00-be02-ac17e2f01da7) |
